...
class TD3_model():
    ...
    def __init__(self, act_limit, obs_dim, act_dim, hidden_sizes, pi_lr=1e-3, q_lr=1e-3, gamma=None, alpha=None, polyak=None,
                 load=False, noise_scale =0.1, target_noise=0.2, noise_clip=0.5, policy_delay=2, exp_name='Exp1', replay_buffer=None, path='saved_models/'):
    ...
    def create_networks(self, obs_dim, act_dim, hidden_sizes=[32, 32], batch_size=100, activation=nn.ReLU):
        ...
        self.ac = MLPActorCriticTD3(self.act_limit, obs_dim, act_dim, hidden_sizes=hidden_sizes, activation=activation, noise_scale=self.noise_scale)
    ...
    def update(self, data):
        ...
        if self.update_timer % self.policy_delay == 0:
    ...
    def load_weights(self, path='saved_models/'):
        self.ac.load_state_dict(torch.load(path+'_TD3_'+self.exp_name+'.pth'))
        self.ac.eval()
        print('Successfully loaded')

    def save_weights(self, path='saved_models/'):
        print('Saved at ', path+'_TD3_'+self.exp_name+'.pth')
        torch.save(self.ac.state_dict(), path+'_TD3_'+self.exp_name+'.pth')
...
def TD3(env_fn, ac_kwargs=dict(), seed=0,
    ...
    TD3 = TD3_model(act_limit, obs_dim, act_dim, ac_kwargs['hidden_sizes'], pi_lr = lr, q_lr = lr, gamma = gamma, alpha = alpha, polyak =polyak,
                    load=load,
                    exp_name=exp_name, replay_buffer=replay_buffer)
    ...
    summary_writer = None#tf.summary.create_file_writer(train_log_dir)
